...
export async function GET(req: Request) {
  try {
    ...
    if (rulesErr) {
      return NextResponse.json({ success: false, error: 'rules fetch error', details: rulesErr });
    }
    ...
    // Probar a subir solo la primera habitación para diagnosticar el error
    const testPayload = ratesPayload.slice(0, 1);
    console.log('[Debug Sync] Enviando payload a Beds24 para roomId:', testPayload[0]?.roomId);
    
    try {
      const response = await pushRatesToBeds24(testPayload);
      return NextResponse.json({ success: true, message: 'Beds24 aceptó el payload de prueba', details: response });
    } catch (pushErr: any) {
      return NextResponse.json({ success: false, error: 'Beds24 push error', message: pushErr.message, stack: pushErr.stack });
    }
