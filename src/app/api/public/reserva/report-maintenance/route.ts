...
export async function POST(req: Request) {
  try {
    ...
    // 5. Enviar plantilla de WhatsApp al gerente de mantenimiento (+52 958 119 8448)
    ...
    console.log(`[Report Maintenance] Sending WA template notification to manager: ${managerPhone}`);
    const waRes = await sendWhatsAppTemplate(
      managerPhone,
      'incidencia_mantenimiento',
      parameters,
      undefined,
      undefined, // bookingId (evitar silenciar por preferencias del huésped)
      undefined,
      true // bypassPause
    );
    if (!waRes.success) {
      console.warn("[Report Maintenance] WhatsApp manager template notification warning:", waRes.error);
    }
