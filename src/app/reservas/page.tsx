...
function ReservasListInner() {
  ...
  useEffect(() => {
    if (showReassignModal && reassigningRes && targetRoomName) {
      ...
      const resNights = Math.max(1, reassigningRes.nights || (reassigningRes.check_in && reassigningRes.check_out ? Math.round((new Date(reassigningRes.check_out + 'T12:00:00').getTime() - new Date(reassigningRes.check_in + 'T12:00:00').getTime()) / (1000 * 60 * 60 * 24)) : 1));
        ...
        const fetchPreview = async () => {
          ...
          try {
            ...
            if (data.success && data.recalculated_price !== undefined) {
              ...
              const currentTotal = Number(reassigningRes.price_estimate || reassigningRes.price || 0);
  ...
  const handleSendUltimoAviso = async (targetRes: any) => {
    ...
    try {
      ...
      // 1. Actualizar estado en Supabase para todos los miembros para indicar aviso enviado y marcado revisado
      for (const member of members) {
        await supabase
          .from('beds24_reservations')
          .update({ last_notice_sent: true, is_acknowledged: true })
      ...
      {selectedRes && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 backdrop-blur-sm p-0 sm:p-4 transition-all duration-300">
          ...
          >
            ...
            {/* Contenido Modal */}
            <div className="flex-1 overflow-y-auto overscroll-y-contain p-6 pb-28 space-y-6">
              ...
              {isEditingRes ? (
                ...
                <>
                  ...
                  {selectedRes && selectedRes.id !== 'walkin' && (
              ...
              {/* Botones de Acción (Movidos adentro del Scroll para evitar que se corten en móvil) */}
              <div className="pt-4 border-t border-zinc-150 flex flex-col gap-2 mt-4">
