...
export default function RecepcionPage() {
  ...
  const handleUpdateRoomStatus = async (newStatus: string) => {
    ...
    try {
      ...
      if (json.success) {
        ...
        let logDetails = `Cambió el estado de Habitación ${selectedRoomForStatus.room_number} a '${newStatus}' desde Recepción`;
        if (newStatus === 'limpieza_profunda') {
          logDetails = `Forzó limpieza forzada en Habitación ${selectedRoomForStatus.room_number} · Reportado por ${operatorName}`;
        } else if (newStatus === 'disponible' && prevDbStatus === 'limpieza_profunda') {
          logDetails = `Finalizó limpieza forzada en Habitación ${selectedRoomForStatus.room_number} · Reportado por ${operatorName}`;
        }
    ...
    if (!paymentMode) {
      ...
      const compatible = accounts.filter(acc => {
        ...
        if (paymentMode === 'efectivo') {
          if (isUSD) {
            const isUSDAcc = acc.currency?.toUpperCase() === 'USD';
            return name === 'EFE USD' || (isUSDAcc && (name.includes('EFE') || name.includes('CASH')));
          }
          return name === 'EFECTIVO' || name === 'EFE PEND' || name === 'EFE HUX' || name === 'EFE TRC';
        }
      ...
      if (compatible.length > 0) {
        if (isUSD && paymentMode === 'efectivo') {
          const efeUsd = compatible.find(a => a.name.trim().toUpperCase() === 'EFE USD') || compatible[0];
          setSelectedAccountId(efeUsd.id);
        } else {
          setSelectedAccountId(compatible[0].id);
        }
      } else {
        if (isUSD && paymentMode === 'efectivo') {
          const fallbackEfe = accounts.find(a => a.name.trim().toUpperCase() === 'EFE USD' || a.name.trim().toUpperCase() === 'EFECTIVO');
          setSelectedAccountId(fallbackEfe?.id || '');
        } else {
          setSelectedAccountId('');
        }
      }
    }
  }, [paymentMode, accounts, selectedReserva?.guest_name]);
  ...
  }, [showCheckInModal, selectedReserva?.id, payGroupConsolidated, directGroupTotalBalance, selectedReserva?.guest_name]);
  ...
  const fetchData = async (bypassCache = false) => {
    ...
    try {
      ...
      // Cargar configuracion de temporadas desde Supabase
      if (seasonRes.data && seasonRes.data.value) {
        try {
          const parsed = typeof seasonRes.data.value === 'string' ? JSON.parse(seasonRes.data.value) : seasonRes.data.value;
  ...
  const processCheckIn = async () => {
    ...
    const registerSingleDirectPayment = async (
      ...
      const isUSD = (selectedReserva?.guest_name || '').toUpperCase().includes('(US DOLLARS)') || (typeof baseDesc === 'string' && baseDesc.toUpperCase().includes('(US DOLLARS)'));
      const usdProcessFee = (isUSD && mode === 'efectivo') ? Math.round(cleanAmountNum * 0.05 * 100) / 100 : 0;
      const totalReceived = cleanAmountNum + usdProcessFee;

      let targetAccountId = accountId;
      if (isUSD && mode === 'efectivo') {
        const efeUsd = accounts.find(a => a.name.trim().toUpperCase() === 'EFE USD') || accounts.find(a => a.currency?.toUpperCase() === 'USD' && (a.name.includes('EFE') || a.name.includes('CASH')));
        if (efeUsd) targetAccountId = efeUsd.id;
      }

      const safeDateStr = todayStr || new Date().toLocaleDateString('sv-SE');
      const matchedAccName = accounts.find(a => a.id === targetAccountId)?.name || 'Desconocido';
      ...
      if (usdProcessFee > 0) {
        const feeDesc = `${selectedReserva.guest_name || 'Huésped'} (ID: ${resId}) - Hab ${roomName} - Process Fee 5% Efectivo USD (Sobre: ${selectedEnvelope || 'Efectivo USD'}) (Operado por: ${operatorName})`;
        const { error: feeErr } = await supabase.from('finances').insert({
          type: 'ingreso',
          amount: usdProcessFee,
        ...
        if (feeErr) {
          console.error("Error al registrar Process Fee USD en Finanzas:", feeErr);
        }
      }

      if (targetAccountId) {
        const matchedAcc = accounts.find(a => a.id === targetAccountId);
        if (matchedAcc) {
          const newBalance = matchedAcc.balance + totalReceived;
      ...
      if (emp && !isGroupWalkin) {
        const logDetails = usdProcessFee > 0
          ? `${selectedReserva.guest_name || 'Huésped'} ${selectedReserva.num_adult || 1}/${selectedReserva.num_child || 0} (ID: ${resId}) de la Habitación ${roomName} - Recibió cobro Check-in USD en efectivo: $${cleanAmountNum} USD + $${usdProcessFee} USD (Process Fee 5%) = Total $${totalReceived} USD (Depositado en sobre: ${selectedEnvelope || matchedAccName}).`
          : `${selectedReserva.guest_name || 'Huésped'} ${selectedReserva.num_adult || 1}/${selectedReserva.num_child || 0} (ID: ${resId}) de la Habitación ${roomName} - Recibió pago de $${cleanAmountNum} vía ${mode} (Depositado en sobre: ${matchedAccName}).`;
    ...
    if (selectedReserva.id === 'walkin') {
      ...
      if (payGroupConsolidated) {
          ...
          if (isOtaAutomated) {
            ...
            if (netRevenue > 0) {
              ...
              if (netAcc) {
                ...
                setAccounts(prev => prev.map(a => a.id === netAcc.id ? { ...a, balance: newBalance } : a));
              }
            }
            ...
            if (taxesRetained > 0) {
              ...
              const taxDesc = `${r.guest_name || 'Huésped'} (ID: ${r.id}) - Hab ${r.room} - Retención de Impuestos ${channel}`;
              await supabase.from('finances').insert({
                type: 'gasto',
            ...
            if (splitAmt > 0) {
              ...
              if (otaSplit.isOTA) {
                ...
                if (netErr) {
                  ...
                  if (cleanCommission > 0) {
                    ...
                    if (commErr) {
                      ...
                      setAccounts(prev => prev.map(a => a.id === commissionAcc.id ? { ...a, balance: newCommBalance } : a));
                    }
                  }

                  // Registro contable de retención de impuestos grupal
                  const cleanTaxesRetained = Number(otaSplit.taxesRetained) || 0;
                  if (cleanTaxesRetained > 0) {
                    const taxAcc = accounts.find(a => (a.name || '').toUpperCase().includes('RETENCIÓN') || (a.name || '').toUpperCase().includes('RETENCION'));
                    const { error: taxErr } = await supabase.from('finances').insert({
                      type: 'gasto',
                      amount: cleanTaxesRetained,
                      category: 'Impuestos',
                      description: `${r.guest_name || 'Huésped'} (ID: ${r.id}) - Hab ${r.room} - Retención de Impuestos ${otaSplit.channelLabel}`,
                      payment_method: 'transferencia',
                      account_id: taxAcc?.id || null,
                      date: safeDateStr
                    });
                    ...
                    if (taxErr) {
                      console.error("Error al registrar egreso retención impuestos OTA (grupo-directo):", taxErr);
                      alert(`⚠️ Error al registrar la retención en Finanzas para la Habitación ${r.room}: ${taxErr.message}`);
                    } else if (taxAcc) {
                      const newTaxBalance = taxAcc.balance + cleanTaxesRetained;
                      await supabase.from('accounts').update({ balance: newTaxBalance }).eq('id', taxAcc.id);
                      setAccounts(prev => prev.map(a => a.id === taxAcc.id ? { ...a, balance: newTaxBalance } : a));
                    }
                  }
                ...
                const isUSD = (r.guest_name || selectedReserva.guest_name || '').toUpperCase().includes('(US DOLLARS)');
                const usdProcessFee = (isUSD && paymentMode === 'efectivo') ? Math.round(cleanSplitAmt * 0.05 * 100) / 100 : 0;
                const totalReceived = cleanSplitAmt + usdProcessFee;
                ...
                if (insertErr) {
                  ...
                  if (usdProcessFee > 0) {
                    const feeDesc = `${r.guest_name || 'Huésped'} (ID: ${r.id}) - Hab ${r.room} - Process Fee 5% Efectivo USD (Sobre: ${selectedEnvelope || 'Efectivo USD'}) (Operado por: ${operatorName})`;
                    const { error: feeErr } = await supabase.from('finances').insert({
                      type: 'ingreso',
                      amount: usdProcessFee,
                      category: 'Comisión de Procesamiento',
                    ...
                    if (feeErr) {
                      console.error("Error al registrar Process Fee USD en Finanzas (grupo):", feeErr);
                    }
                  }
                ...
                if (emp) {
                  const matchedAccName = accounts.find(a => a.id === targetAccountId)?.name || 'Desconocido';
                  const logDetails = usdProcessFee > 0
                    ? `${r.guest_name || 'Huésped'} ${r.num_adult || 1}/${r.num_child || 0} (ID: ${r.id}) de la Habitación ${r.room} (Grupo Consolidado) - Recibió cobro Check-in USD en efectivo: $${cleanSplitAmt} USD + $${usdProcessFee} USD (Process Fee 5%) = Total $${totalReceived} USD (Depositado en sobre: ${selectedEnvelope || matchedAccName}).`
                    : `${r.guest_name || 'Huésped'} ${r.num_adult || 1}/${r.num_child || 0} (ID: ${r.id}) de la Habitación ${r.room} (Grupo Consolidado) - Recibió pago de $${splitAmt} vía ${paymentMode} (Depositado en sobre: ${matchedAccName}).`;
        ...
        if (isOtaAutomated && (!paymentAmount || Number(paymentAmount) === 0)) {
          ...
          if (taxesRetained > 0) {
            const taxAcc = accounts.find(a => (a.name || '').toUpperCase().includes('RETENCIÓN') || (a.name || '').toUpperCase().includes('RETENCION'));
            const taxDesc = `${selectedReserva.guest_name || 'Huésped'} (ID: ${selectedReserva.id}) - Hab ${selectedReserva.room} - Retención de Impuestos ${channel}`;
            await supabase.from('finances').insert({
              type: 'gasto',
              amount: taxesRetained,
              category: 'Impuestos',
          ...
          if (otaSplit.isOTA) {
            ...
            if (otaSplit.taxesRetained > 0) {
              const taxAcc = accounts.find(a => (a.name || '').toUpperCase().includes('RETENCIÓN') || (a.name || '').toUpperCase().includes('RETENCION'));
              await supabase.from('finances').insert({
                type: 'gasto',
                amount: otaSplit.taxesRetained,
                category: 'Impuestos',
                description: `${selectedReserva.guest_name || 'Huésped'} (ID: ${selectedReserva.id}) - Hab ${selectedReserva.room} - Retención de Impuestos ${otaSplit.channelLabel}`,
                payment_method: 'transferencia',
                account_id: taxAcc?.id || null,
                date: todayStr
              });
              ...
              if (taxAcc) {
                const newTaxBalance = taxAcc.balance + otaSplit.taxesRetained;
                await supabase.from('accounts').update({ balance: newTaxBalance }).eq('id', taxAcc.id);
                setAccounts(prev => prev.map(a => a.id === taxAcc.id ? { ...a, balance: newTaxBalance } : a));
              }
            }
            ...
            const isUSD = (selectedReserva?.guest_name || '').toUpperCase().includes('(US DOLLARS)');
            const usdProcessFee = (isUSD && paymentMode === 'efectivo') ? Math.round(cleanAmountNum * 0.05 * 100) / 100 : 0;
            const totalReceived = cleanAmountNum + usdProcessFee;
            ...
            if (insertErr) {
              ...
              if (usdProcessFee > 0) {
                const feeDesc = `${selectedReserva.guest_name || 'Huésped'} (ID: ${selectedReserva.id}) - Hab ${selectedReserva.room} - Process Fee 5% Efectivo USD (Sobre: ${selectedEnvelope || 'Efectivo USD'}) (Operado por: ${operatorName})`;
                const { error: feeErr } = await supabase.from('finances').insert({
                  type: 'ingreso',
                  amount: usdProcessFee,
                  category: 'Comisión de Procesamiento',
            ...
            if (emp) {
              const matchedAccName = accounts.find(a => a.id === targetAccountId)?.name || 'Desconocido';
              const logDetails = usdProcessFee > 0
                ? `${selectedReserva.guest_name || 'Huésped'} ${selectedReserva.num_adult || 1}/${selectedReserva.num_child || 0} (ID: ${selectedReserva.id}) de la Habitación ${selectedReserva.room} - Recibió cobro Check-in USD en efectivo: $${cleanAmountNum} USD + $${usdProcessFee} USD (Process Fee 5%) = Total $${totalReceived} USD (Depositado en sobre: ${selectedEnvelope || matchedAccName}).`
                : `${selectedReserva.guest_name || 'Huésped'} ${selectedReserva.num_adult || 1}/${selectedReserva.num_child || 0} (ID: ${selectedReserva.id}) de la Habitación ${selectedReserva.room} - Recibió pago de $${paymentAmount} vía ${paymentMode} (Depositado en sobre: ${matchedAccName}).`;
                        ...
                        {(() => {
                          ...
                          return (
                            <>
                              ...
                              {isUSD ? (
                                <div className="space-y-3">
                                  {/* Banner Informativo Exclusivo USD */}
                                  <div className="p-3 bg-emerald-50/90 border border-emerald-200/90 rounded-xl flex items-center justify-between">
                                    <div className="flex items-center gap-2.5">
                                      ...
                                      <div className="text-left">
                                        <p className="text-[12px] font-bold text-emerald-950 uppercase tracking-wide">Efectivo en Dólares (USD)</p>
                                        <p className="text-[10px] text-emerald-700 font-medium">Único método de cobro presencial en recepción para reservaciones en USD</p>
                                      </div>
                                    </div>
                                    <span className="px-2 py-0.5 bg-emerald-200/80 text-emerald-900 text-[10px] font-black rounded-full uppercase tracking-wider">
                                      USD CASH ONLY
                                    </span>
                                  </div>

                                  {/* Desglose de Cobro con 5% Process Fee */}
                                  {(() => {
                                    const cleanAmountNum = Number(paymentAmount) || 0;
                                    const usdProcessFee = Math.round(cleanAmountNum * 0.05 * 100) / 100;
                                    const totalToCollect = Math.round((cleanAmountNum + usdProcessFee) * 100) / 100;

                                    return (
                                      <div className="space-y-3 p-3.5 bg-zinc-50 border border-zinc-200/80 rounded-2xl animate-in fade-in duration-200">
                                        <div className="flex justify-between items-center">
                                          <span className="text-[10px] font-extrabold text-zinc-500 uppercase tracking-wider">
                                            Monto Base de Estancia a Abonar (USD)
                                          </span>
                                        </div>
                                        ...
                                        <div className="relative text-left">
                                          ...
                                          <input
                                            ...
                                            placeholder="0.00"
                                            className="w-full bg-[#fafafa] border border-zinc-200/80 focus:bg-white focus:border-zinc-400 focus:ring-4 focus:ring-zinc-900/5 text-zinc-900 font-bold text-[16px] shadow-sm rounded-xl p-3.5 pl-8 pr-14 transition-all outline-none"
                                          />
                                          <span className="absolute right-3.5 top-1/2 -translate-y-1/2 font-extrabold text-xs text-zinc-400">USD</span>
                                        </div>

                                        {/* Tarjeta de desglose Process Fee */}
                                        <div className="bg-amber-50/90 border border-amber-200/90 rounded-xl p-3.5 text-left space-y-2.5">
                                          <div className="flex items-center gap-1.5 text-amber-800 font-bold text-[11px] uppercase tracking-wider">
                                            <AlertTriangle size={14} className="text-amber-600 shrink-0" />
                                            <span>Desglose de Cobro Check-In (Process Fee 5%)</span>
                                          </div>
                                          
                                          <div className="space-y-1.5 text-xs divide-y divide-amber-200/60 font-medium">
                                            <div className="flex justify-between py-1 text-zinc-600">
                                              <span>Abono neto a estancia (Beds24):</span>
                                              <span className="font-bold text-zinc-900">${cleanAmountNum.toFixed(2)} USD</span>
                                            </div>
                                            <div className="flex justify-between py-1 text-amber-800">
                                              <span>Process Fee (+5% Efectivo USD):</span>
                                              <span className="font-bold text-amber-800">+${usdProcessFee.toFixed(2)} USD</span>
                                            </div>
                                            <div className="flex justify-between pt-1.5 font-extrabold text-[13px] text-zinc-950">
                                              <span>TOTAL EN EFECTIVO A COBRAR:</span>
                                              <span className="text-emerald-700 text-sm font-black">${totalToCollect.toFixed(2)} USD</span>
                                            </div>
                                          </div>

                                          <p className="text-[10px] text-amber-900/80 pt-1 leading-normal border-t border-amber-200/60">
                                            * Cobrar <strong>${totalToCollect.toFixed(2)} USD</strong> en efectivo. A la reservación se abonará el saldo neto (<strong>${cleanAmountNum.toFixed(2)} USD</strong>) y el 5% de comisión (<strong>${usdProcessFee.toFixed(2)} USD</strong>) se registrará en Finanzas.
                                          </p>
                                        </div>
                                        ...
                                        <div className="space-y-1.5 pt-1 text-left">
                                          <label className="text-[11px] font-semibold text-zinc-500 uppercase tracking-widest pl-0.5 mb-1.5 block flex items-center gap-1">
                                            ¿En qué sobre va a colocarse el efectivo?
                                          ...
                                          <select
                                            ...
                                            required
                                            className={`w-full border rounded-xl p-3.5 text-zinc-900 font-semibold text-[15px] focus:bg-white focus:border-zinc-400 focus:ring-4 focus:ring-zinc-900/5 transition-all outline-none cursor-pointer ${
                                              !selectedEnvelope
                                        ...
                                        <div className="space-y-1.5 pt-1 text-left">
                                          ...
                                          <input
                                            ...
                                            placeholder="Ej. S07 -EP, billetes en USD..."
              ...
              {isCleanTerminated ? (
                ...
                <div className="space-y-5">
                  ...
                  <div className="flex flex-col gap-2.5 pt-2">
                    ...
                    <button
                      ...
                      className="w-full bg-rose-50 hover:bg-rose-100 text-rose-600 border border-rose-200 font-bold text-[12px] py-3.5 rounded-xl transition-colors cursor-pointer flex items-center justify-center gap-1.5"
                    >
                      <Wrench size={14} />
                ...
                <div className="space-y-5">
                  ...
                  <div className="pt-2 space-y-2">
                    {(operStatus === 'sucio_checkout' || operStatus === 'en_limpieza' || operStatus === 'limpieza_programada' || operStatus === 'limpieza_profunda') && (
                      operStatus === 'limpieza_profunda' ? (
                        <button
                          onClick={() => runWithSignature('room_status', () => handleUpdateRoomStatus('disponible'))}
                          className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold text-[12px] tracking-wide uppercase py-4 rounded-2xl transition-all cursor-pointer flex items-center justify-center gap-2 shadow-md active:scale-[0.98]"
                        >
                    ...
                    {(operStatus === 'ocupada' || operStatus === 'disponible') && (
                      <button
                        onClick={() => runWithSignature('room_status', () => handleUpdateRoomStatus('limpieza_profunda'))}
                        className="w-full bg-orange-50 hover:bg-orange-100 text-orange-700 border border-orange-300 font-extrabold text-[12px] tracking-wide uppercase py-4 rounded-2xl transition-all cursor-pointer flex items-center justify-center gap-2 shadow-sm active:scale-[0.98]"
                      >
                      ...
                      return (
                        <div
                          ...
                          onClick={() => {
                            ...
                            const foundRes = reservas.find(r => matchesRoomNumber(r, roomNum) && r.status !== 'cancelled');
                            if (foundRes) {
                              setSelectedReserva(foundRes);
                              setShowCheckInModal(true);
                            }
                          }}
