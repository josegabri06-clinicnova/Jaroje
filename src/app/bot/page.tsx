...
function parseTemplateMessage(text: string) {
  ...
  const isTemplate = text.includes('[Botón:') || text.includes('[Button:') || text.includes('🔗') || text.includes('⚡ Plantilla') || text.includes('[Plantilla:');

  // Si es un marcador de plantilla crudo: [Plantilla: nombre] (Parámetros: p1, p2)
  if (text.startsWith('[Plantilla:')) {
    const tNameMatch = text.match(/\[Plantilla:\s*([^\]]+)\]/);
    const tName = tNameMatch ? tNameMatch[1].trim() : '';
    
    let params: string[] = [];
    const paramsMatch = text.match(/\(Parámetros:\s*([^\)]+)\)/);
    if (paramsMatch) {
      params = paramsMatch[1].split(',').map(s => s.trim());
    }
    
    const name = params[0] || 'Huésped';
    
    // Traducir el marcador crudo a texto de plantilla local
    switch (tName) {
  ...
  let match;
  while ((match = buttonRegex.exec(resolvedText)) !== null) {
    buttons.push(match[1].trim());
  }

  let cleanText = resolvedText.replace(/\[(?:Botón|Button):\s*([^\]]+)\]/g, '').trim();
