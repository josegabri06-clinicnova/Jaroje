...
export function getChannelMultiplier(
  referer: string, 
  customMultipliers?: { airbnb?: number; booking?: number; google?: number; expedia?: number; directo?: number }
): number {
  ...
  if (r.includes('google')) return multGoogle;
  if (r.includes('expedia')) return multExpedia;
  ...
  const doFetchAttempt = async (token: string) => {
    ...
    if (includeCancelled) {
      ...
      const [activeBookings, recentCancelledBookings] = await Promise.all([
        ...
        fetchPage(`${baseUrl}&status=cancelled&modifiedFrom=${fortyEightHoursAgoIso}`).catch(err => {
...
export async function checkIfOtaIsAlreadyAdjusted(
  ...
  otaMultipliers: { airbnb: number; booking: number; google?: number; expedia?: number },
  allBookings?: any[]
): Promise<boolean> {
...
async function doFetchAndMapBeds24Bookings(fast: boolean = false, includeCancelled: boolean = false): Promise<any[]> {
  ...
  const otaRoom500Bookings = bookingsArray.filter((b: any) => {
    ...
    return {
      ...
      phone: normalizePhone(b.phone || b.mobile || b.guestPhone || b.guestMobile || '', b.country2 || b.country || b.guestCountry2 || b.guestCountry),
